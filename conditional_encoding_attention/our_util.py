...
def split_data(data, prop_train = 0.4, prop_dev = 0.2, seed = None):
...
def split_indices(num_samples, prop_train = 0.4, prop_dev = 0.2):
    num_train_samples = int(np.floor(num_samples * prop_train))
    num_dev_samples = int(np.floor(num_samples * prop_dev))
    indices = range(num_samples)
    np.random.shuffle(indices)
...
def get_performance(predicted, truth, n_classes = None, outputStyle = 'dict'):
    ...
    print 'competition score ' + str(competition_score)
    return output
