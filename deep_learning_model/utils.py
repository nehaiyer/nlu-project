...
def split():
    ...
    train = []
    test = []
    with open('train_ids.txt','r',errors='replace') as f:
        train_sets = set([int(i.strip()) for i in f.readlines()])
    with open('test_ids.txt','r',errors='replace') as f:
        test_sets = set([int(i.strip()) for i in f.readlines()])
    print (len(train_sets), len(test_sets))
    with open('train_stances.csv','r',errors='replace') as f:
        reader = csv.reader(f)
        reader.__next__()
        for l in reader:
...
class News(object):
    ...
    def __init__(self, stances='train_stances.csv',bodies='train_bodies.csv',vecs=None):
        ...
        with open(stances,'r',errors='replace') as f:
            reader = csv.reader(f)
            reader.__next__()
            for line in reader:
                if len(line)==2:
