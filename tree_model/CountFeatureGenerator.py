...
class CountFeatureGenerator(FeatureGenerator):
    ...
    def process(self, df):
        ...
        for rf in check_words:
            ...
            df[fname] = list(map(lambda x: 1 if rf in x else 0,df['Headline']))
        ...
        with open(outfilename_bcf_train, "wb") as outfile:
            _pickle.dump(feat_names, outfile, -1)
            _pickle.dump(xBasicCountsTrain, outfile, -1)
        ...
        if test.shape[0] > 0:
            ...
            with open(outfilename_bcf_test, 'wb') as outfile:
                _pickle.dump(feat_names, outfile, -1)
                _pickle.dump(xBasicCountsTest, outfile, -1)
    ...
    def read(self, header='train'):
        ...
        with open(filename_bcf, "rb") as infile:
            feat_names = _pickle.load(infile)
            xBasicCounts = _pickle.load(infile)
            print('feature names: ')
            print(feat_names)
