...
class SentimentFeatureGenerator(FeatureGenerator):
    ...
    def process(self, df):
        ...
        with open(outfilename_hsenti_train, "wb") as outfile:
            _pickle.dump(headlineSentiTrain, outfile, -1)
        ...
        if n_test > 0:
            ...
            with open(outfilename_hsenti_test, "wb") as outfile:
                _pickle.dump(headlineSentiTest, outfile, -1)
        ...
        df['body_sents'] = list(map(lambda x: sent_tokenize(x),df['articleBody']))
        ...
        with open(outfilename_bsenti_train, "wb") as outfile:
            _pickle.dump(bodySentiTrain, outfile, -1)
        ...
        if n_test > 0:
            ...
            with open(outfilename_bsenti_test, "wb") as outfile:
                _pickle.dump(bodySentiTest, outfile, -1)
