...
class TfidfFeatureGenerator(FeatureGenerator):
    ...
    def process(self, df):
        ...
        xHeadlineTfidf = vecH.fit_transform(list(map(lambda x: ' '.join(x),df['Headline_unigram']))) # use ' '.join(Headline_unigram) instead of Headline since the former is already stemmed
        ...
        with open(outfilename_htfidf_train, "wb") as outfile:
            _pickle.dump(xHeadlineTfidfTrain, outfile, -1)
        ...
        if n_test > 0:
            ...
            with open(outfilename_htfidf_test, "wb") as outfile:
                _pickle.dump(xHeadlineTfidfTest, outfile, -1)
        ...
        xBodyTfidf = vecB.fit_transform(list(map(lambda x: ' '.join(x),df['articleBody_unigram'])))
        ...
        with open(outfilename_btfidf_train, "wb") as outfile:
            _pickle.dump(xBodyTfidfTrain, outfile, -1)
        ...
        if n_test > 0:
            ...
            with open(outfilename_btfidf_test, "wb") as outfile:
                _pickle.dump(xBodyTfidfTest, outfile, -1)
        ...
        with open(outfilename_simtfidf_train, "wb") as outfile:
            _pickle.dump(simTfidfTrain, outfile, -1)
        ...
        if n_test > 0:
            ...
            with open(outfilename_simtfidf_test, "wb") as outfile:
                _pickle.dump(simTfidfTest, outfile, -1)
