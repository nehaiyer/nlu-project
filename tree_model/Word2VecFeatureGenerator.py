...
class Word2VecFeatureGenerator(FeatureGenerator):
    ...
    def process(self, df):
        ...
        df["Headline_unigram_vec"] = list(map(lambda x: preprocess_data(x, exclude_stopword=False, stem=False),df["Headline"]))
        df["articleBody_unigram_vec"] = list(map(lambda x: preprocess_data(x, exclude_stopword=False, stem=False),df["articleBody"]))
        ...
        with open(outfilename_hvec_train, "wb") as outfile:
            _pickle.dump(headlineVecTrain, outfile, -1)
        ...
        if n_test > 0:
            ...
            with open(outfilename_hvec_test, "wb") as outfile:
                _pickle.dump(headlineVecTest, outfile, -1)
        ...
        with open(outfilename_bvec_train, "wb") as outfile:
            _pickle.dump(bodyVecTrain, outfile, -1)
        ...
        if n_test > 0:
            ...
            with open(outfilename_bvec_test, "wb") as outfile:
                _pickle.dump(bodyVecTest, outfile, -1)
        ...
        with open(outfilename_simvec_train, "wb") as outfile:
            _pickle.dump(simVecTrain, outfile, -1)
        ...
        if n_test > 0:
            ...
            with open(outfilename_simvec_test, "wb") as outfile:
                _pickle.dump(simVecTest, outfile, -1)
