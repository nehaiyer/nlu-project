...
def cv():
    ...
    data_x, data_y, body_ids = build_data()
    fp1 = open('hold_out_ids.txt')
    fp2 = open('training_ids.txt')
    #holdout_ids = set([int(x.rstrip()) for x in file('hold_out_ids.txt')])
    holdout_ids = set([int(x.rstrip()) for x in fp1])
    ...
    cv_ids = set([int(x.rstrip()) for x in fp2])
...
def show_incorrect_pred(actual, predicted, idx_valid):
    ...
    data['actual'] = actual
    data['articleBody'] = list(map(lambda s: s.replace("\n",""),data['articleBody']))
